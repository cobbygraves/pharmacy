...
import useShowDrugs from "../custom-hooks/ShowDrugsHook";
...
const Home = () => {
  ...
  const showModalHandler = (drugId, drugName) => {
    ...
    // const drug = dailyDrugs.find((eachDrug) => eachDrug.id === drugId);
    // const price = drug.price;
    // setTotalSales((prevSales) => prevSales - price);
    // const newDailyDrugs = dailyDrugs.filter(
    //   (eachDrug) => eachDrug.id !== drugId
    // );
                          ...
                          if (drug.left == 0) {
                            return (
                              <Drug
                                ...
                                // drugInfo={() => updateDrugHandler(drug.id)}
                              />
                            );
                          } else {
                            return (
